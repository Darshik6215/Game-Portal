...
export default function AddGameModal({ onClose, onSuccess }: AddGameModalProps) {
  ...
  const onSubmit = async (data: GameFormData) => {
    try {
      ...
      const response = await fetchWithAuth(`${apiUrl}/api/v1/games`, {
        method: 'POST',
        headers: {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
            ...
            <label className="block text-sm font-semibold mb-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (slugInput) slugInput.value = generateSlug();
                }}
          ...
          <div>
            ...
            <input
              {...register('iframe_url')}
              type="url"
              placeholder="https://example.com/game/subway-surfers"
              className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all"
            />
            {errors.iframe_url && (
          ...
          <div>
            ...
            <input
              {...register('thumbnail')}
              type="url"
              placeholder="https://example.com/thumbnails/game.jpg"
              className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all"
            />
            {errors.thumbnail && (
